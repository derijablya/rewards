...
class RequestRewardView(APIView):
    def post(self, request):
        if RewardLog.objects.filter(
            user=request.user, given_at__date=timezone.now().date()
        ).exists():
            return Response(
        ...
        reward = ScheduledReward.objects.create(
            user=request.user, amount=10, execute_at=execute_at
        )

        process_scheduled_reward.apply_async(args=[reward.id], eta=execute_at)
